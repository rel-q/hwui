...
struct thread_data_t {
    ...
    static int trampoline(const thread_data_t* t) {
        ...
        if (prio >= ANDROID_PRIORITY_BACKGROUND) {
            setpriority(PRIO_PROCESS, 0, SP_BACKGROUND);
        } else {
            setpriority(PRIO_PROCESS, 0, SP_FOREGROUND);
        }
...
int androidSetThreadPriority(pid_t tid, int pri)
{
    ...
    if (pri >= ANDROID_PRIORITY_BACKGROUND) {
        rc = setpriority(PRIO_PROCESS, tid, SP_BACKGROUND);
    } else if (getpriority(PRIO_PROCESS, tid) >= ANDROID_PRIORITY_BACKGROUND) {
        rc = setpriority(PRIO_PROCESS, tid, SP_FOREGROUND);
    }
...
pid_t Thread::getTid() const
{
    ...
    if (mRunning) {
        ...
        pthread_threadid_np(pthread, &id);
        tid = (pid_t) id;
    } else {
